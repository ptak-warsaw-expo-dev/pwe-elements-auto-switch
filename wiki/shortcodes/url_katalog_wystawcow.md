...
Zwraca URL strony opisanej kluczem `katalog_wystawcow` w języku bieżącym lub podanym przez atrybut `lang`. Może zwrócić adres absolutny po ustawieniu `absolute=true`.
...
- `pl` → `/katalog-wystawcow/` (Katalog wystawców)
- `en` → `/exhibitors-catalog/` (Exhibitor Directory)
- `de` → `/ausstellerverzeichnis/` (Ausstellerverzeichnis)
- `uk` → `/katalog-eksponentiv/` (Каталог експонентів)
