...
Zwraca URL strony opisanej kluczem `zostan_wystawca` w języku bieżącym lub podanym przez atrybut `lang`. Może zwrócić adres absolutny po ustawieniu `absolute=true`.
...
- `pl` → `/zostan-wystawca/` (Zostań wystawcą)
- `en` → `/become-an-exhibitor/` (Become an Exhibitor)
- `de` → `/werden-sie-aussteller/` (Werden Sie Aussteller)
- `uk` → `/staty-eksponentom/` (Стати експонентом)
