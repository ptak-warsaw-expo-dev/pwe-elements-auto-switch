...
**Callback:** `show_trade_fair_name_eng`  
**Źródło:** `includes/class-shortcodes.php:2315`

## Krótki opis

Zwraca angielską nazwę targów.

## Jak działa

1. Shortcode jest rejestrowany podczas hooka `init` (priorytet 20) przez `PWE_Shortcodes::register_shortcodes()`.
2. WordPress wywołuje callback `show_trade_fair_name_eng`.
3. Callback korzysta z innych shortcode’ów jako źródła/fallbacku: `[pwe_name_en]`, `[pwe_name_pl]`.
4. Odczytuje ustawienia WordPress: `pwe_general_options`, `trade_fair_name_eng`.
