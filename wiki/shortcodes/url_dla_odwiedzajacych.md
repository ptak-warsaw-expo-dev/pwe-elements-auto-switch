...
Zwraca URL strony opisanej kluczem `dla_odwiedzajacych` w języku bieżącym lub podanym przez atrybut `lang`. Może zwrócić adres absolutny po ustawieniu `absolute=true`.
...
- `pl` → `/dla-odwiedzajacych/` (Dla odwiedzających)
- `en` → `/for-visitors/` (For Visitors)
- `de` → `/fur-besucher/` (Für Besucher)
- `uk` → `/dlya-vidviduvachiv/` (Для відвідувачів)
