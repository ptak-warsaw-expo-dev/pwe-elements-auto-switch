...
Zwraca URL strony opisanej kluczem `dla_wystawcow` w języku bieżącym lub podanym przez atrybut `lang`. Może zwrócić adres absolutny po ustawieniu `absolute=true`.
...
- `pl` → `/dla-wystawcow/` (Dla wystawców)
- `en` → `/for-exhibitors/` (For Exhibitors)
- `de` → `/fur-aussteller/` (Für Aussteller)
- `uk` → `/dlya-eksponentiv/` (Для експонентів)
