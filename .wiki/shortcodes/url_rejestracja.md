...
Zwraca URL strony opisanej kluczem `rejestracja` w języku bieżącym lub podanym przez atrybut `lang`. Może zwrócić adres absolutny po ustawieniu `absolute=true`.
...
- `de` → `/anmeldung/` (Anmeldung)
- `uk` → `/reyestraciya/` (Реєстрація)
